...
describe Idv::UpcaseVendorEnvVars do
  describe '#call' do
    before do
      allow(Figaro.env).to receive(:proofing_vendor_profile).and_return('equifax')
      allow(Figaro.env).to receive(:proofing_vendor_finance).and_return('aamva')
      allow(Figaro.env).to receive(:proofing_vendor_phone).and_return('equifax')
